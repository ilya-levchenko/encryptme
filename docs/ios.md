...
Оба напоминания добровольные и по умолчанию выключены. iOS показывает системный запрос разрешения только после включения переключателя в EncryptMe. Напоминания планируются через локальный `UNUserNotificationCenter`; APNs, сервер и push-токены не используются. Текст дневника никогда не помещается в уведомление.
